...
// =====================================================
// 마스터 메타데이터 (데이터 유실 시 복구용)
// =====================================================
const MASTER_ACN_MAP = {
    "HbA1c": 99999, "AAGP2": 20020, "AAT2": 20030, "ACET2": 20040, "ACP2": 20050, 
    "ALB2-G": 20090, "ALB2-P": 20110, "ALP2": 20120, "ALT2": 20130, "AMYL2": 20140, "PAMY2": 20150,
    "ASAT2": 20160, "BILD2": 20180, "BILT3": 20190, "CA2": 20210, "CHE2": 20230, "CHOL2": 20240,
    "CK2": 20250, "CKMB2": 20260, "CREP2": 20290, "CREJ2": 20300, "CRP4": 20310, "GGT2": 20330,
    "GLUC3": 20340, "HDL-C": 20350, "IRON2": 20370, "LDH2": 20390, "LDL-C": 20400, "LIPC": 20410,
    "MG2": 20430, "PHOS2": 20450, "TP2": 20480, "TRIGL": 20490, "UA2": 20500, "UREAL": 20510,
    "AFP": 10010, "CA125": 10020, "CA15-3": 10030, "CA19-9": 10040, "CEA": 10050, "FPSA": 10080,
    "PSA": 10160, "FERR": 10070, "FOL": 10060, "VITB12": 10180, "T3": 10210, "T4": 10220,
    "FT3": 10090, "FT4": 10100, "TSH": 10172, "HCG": 10120, "HBSAG": 10130, "TP1NP": 10119
};

const MASTER_SEARCH_MAP = {
    "HbA1c": "Haemoglobin A1c (NGSP)" // 사용자 요청: NGSP 기본화
};
...
async function loadInitialData() {
    ...
    const mergeData = (newData) => {
        newData.forEach(newItem => {
            const index = sampleData.findIndex(s => s.test === newItem.test);
            if (index !== -1) {
                // 기존 항목이 있으면 업데이트
                const existing = sampleData[index];
                
                // ACN/Search 복구 로직 (전달받은 값이 비어있으면 마스터/기존값 유지)
                const acn = newItem.acn || existing.acn || MASTER_ACN_MAP[newItem.test] || 0;
                const search = (newItem.test === "HbA1c") ? MASTER_SEARCH_MAP["HbA1c"] : (newItem.search || existing.search || "");
                ...
                sampleData[index] = {
                    ...
                    acn: acn,
                    search: search,
                    // ACN이 있으면 카테고리 자동 설정 (1xxx: Immunology, 2xxx: Chemistry)
                    category: acn ? 
                        (String(acn).startsWith('1') ? 'Immunology' : 'Chemistry') : 
                        (newItem.category || existing.category || 'Chemistry')
                };
            } else {
                // 신규 항목이면 추가
                const acn = newItem.acn || MASTER_ACN_MAP[newItem.test] || 0;
                const search = MASTER_SEARCH_MAP[newItem.test] || newItem.search || "";
                sampleData.push({
                    ...
                    acn: acn,
                    search: search,
                    category: newItem.category || (acn && String(acn).startsWith('1') ? 'Immunology' : 'Chemistry')
                });
            }
        });
    };
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (json.data && Array.isArray(json.data)) {
                mergeData(json.data);
                console.log(`[데이터 로드] 로컬 서버에서 데이터 병합 완료`);
    ...
    if (!loaded) {
        try {
            ...
            if (res.ok) {
                ...
                if (json.data && Array.isArray(json.data)) {
                    mergeData(json.data);
                    console.log(`[데이터 로드] 정적 JSON에서 데이터 병합 완료`);
                    loaded = true;
                }
            }
        } catch (e) {
    ...
    // [추가] 로컬 스토리지에 저장된 사용자 설정(검색어, ACN) 반영
    const customSettingsStr = localStorage.getItem('eflm_custom_settings');
    if (customSettingsStr) {
        try {
            const customSettings = JSON.parse(customSettingsStr);
            // HbA1c 검색어가 이전 IFCC 값이면 NGSP로 강제 교체
            customSettings.forEach(s => {
                if (s.test === 'HbA1c' && s.search === 'Haemoglobin A1c (IFCC)') {
                    s.search = 'Haemoglobin A1c (NGSP)';
                }
            });
            mergeData(customSettings);
            console.log('[데이터 로드] 로컬 스토리지 사용자 설정 반영 완료');
        } catch (e) {
            console.error('사용자 설정 로드 실패:', e);
        }
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    // 1. 모든 수치 데이터 초기화 (사용자 요청: 공백 표시를 위해)
    sampleData.forEach(item => {
        ...
        // 마스터 맵 정보로 기본값 복구 (ACN, Search)
        if (!item.acn && MASTER_ACN_MAP[item.test]) item.acn = MASTER_ACN_MAP[item.test];
        if (MASTER_SEARCH_MAP[item.test]) item.search = MASTER_SEARCH_MAP[item.test];
    });
...
async function generateTEaReport(format = 'excel') {
    ...
    // 최근 업데이트 날짜 및 생성 일시 추가
    const lastUpdate = document.getElementById('eflm-update-time')?.textContent.split(': ')[1] || '-';
    worksheet.addRow(['최근 업데이트 날짜', lastUpdate]);
    worksheet.addRow(['보고서 생성 일시', new Date().toLocaleString()]);
...
async function saveAdminSettings() {
    ...
    acnInputs.forEach(input => { 
        const testName = input.getAttribute('data-test');
        const item = sampleData.find(i => i.test === testName); 
        if (item) { 
            item.acn = input.value; 
            count++; 
        } 
    });
    searchInputs.forEach(input => { 
        const testName = input.getAttribute('data-test');
        const item = sampleData.find(i => i.test === testName); 
        if (item) item.search = input.value; 
    });

    // 1. 로컬 스토리지 저장 (즉시 반영용)
    localStorage.setItem('eflm_custom_settings', JSON.stringify(sampleData.map(i => ({
        test: i.test, search: i.search, acn: i.acn, category: i.category
    }))));

    // 2. 서버 저장 시도 (영구 저장 및 자동 수집용)
    try {
        const res = await fetch(`${SERVER_URL}/api/save-settings`, {
            method: 'POST',
        ...
        if (result.success) {
            alert(`설정이 저장되었습니다. (${result.message})\n이제 GitHub Actions에서도 이 설정을 사용합니다.`);
        } else {
            alert('설정 저장 중 오류 발생: ' + result.error);
        }
    } catch (e) {
        alert('로컬 서버에 저장하지 못했습니다. (서버가 꺼져 있음)\n현재 브라우저에는 임시 저장되었습니다.');
    }
...
function addNewItem() {
    const acn = document.getElementById('new-acn').value.trim();
    const test = document.getElementById('new-test').value.trim();
    const search = document.getElementById('new-search').value.trim();
    const category = document.getElementById('new-category').value;

    if (!test || !search) {
        alert('검사명과 검색어는 필수입니다.');
        return;
    }

    if (sampleData.find(i => i.test === test)) {
        alert('이미 존재하는 검사명입니다.');
        return;
    }
    ...
    // 입력창 초기화
    document.getElementById('new-acn').value = '';
    document.getElementById('new-test').value = '';
    document.getElementById('new-search').value = '';
    
    alert(`신규 항목 [${test}]이(가) 추가되었습니다. '설정 저장하기'를 눌러 영구 저장하세요.`);
    injectAdminTestSelect();
    injectManageSearchData();
    refreshAllTabs();
}
...
function initTheme() {
    ...
    // localStorage에서 테마 복원 (없으면 기본값: 다크 모드 = OFF)
    const savedTheme = localStorage.getItem('eflm_theme');
    if (savedTheme === 'light') {
    ...
    themeSwitch.addEventListener('change', () => {
        if (themeSwitch.checked) {
            document.body.classList.add('light-theme');
            localStorage.setItem('eflm_theme', 'light');
        } else {
            document.body.classList.remove('light-theme');
            localStorage.setItem('eflm_theme', 'dark');
        }
    });
}
// 초기화 로직은 상단 DOMContentLoaded에 통합됨
...
function generateReport(data, config) {
    ...
    <head>
        ...
        <style>
            ...
            h1 { color: #4c51bf; border-bottom: 2px solid #4c51bf; padding-bottom: 10px; margin-bottom: 5px; }
            .update-date { color: #718096; font-size: 0.9rem; margin-bottom: 20px; }
    ...
    <body>
        <h1>RDKR TEa finder 보고서</h1>
        <div class="update-date">최근 업데이트 날짜: ${document.getElementById('eflm-update-time')?.textContent.split(': ')[1] || '-'}</div>
